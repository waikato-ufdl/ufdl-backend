...
class FileContainerViewSet(RoutedViewSet):
    ...
    def get_route(cls) -> routers.Route:
        return routers.Route(
            url=r'^{prefix}/{lookup}/files/(?P<filename>.+)$',
    ...
    def add_file(self, request: Request, pk=None, filename=None):
        """
        ...
        :param filename:    The filename of the file being added.
        ...
        record = container.add_file(filename, request.data['file'].file.read())

        return Response(NamedFileSerialiser().to_representation(record))

    def get_file(self, request: Request, pk=None, filename=None):
        """
        Gets a file from the dataset for download.

        :param request:     The request.
        :param pk:          The primary key of the dataset being accessed.
        :param filename:    The filename of the file being asked for.
        ...
        return Response(container.get_file(filename))

    def delete_file(self, request: Request, pk=None, filename=None):
        """
        Action to add a file to a dataset.

        :param request:     The request containing the file data.
        :param pk:          The primary key of the dataset being accessed.
        :param filename:    The filename of the file being deleted.
        ...
        # Delete the file
        record = container.delete_file(filename)
