...
class SpectrumClassificationDatasetQuerySet(DatasetQuerySet):
    pass


class SpectrumClassificationDataset(Dataset):
    objects = SpectrumClassificationDatasetQuerySet.as_manager()
    ...
    def merge_annotations(self, other: Dataset, files: List[Tuple[FileReference, FileReference]]):
        for source_file_reference, destination_file_reference in files:
            for category in source_file_reference.sc_categories.all():
                label: str = category.category
                if not destination_file_reference.sc_categories.with_category(label).exists():
                    Category(
    ...
    def delete_file(self, filename: str):
        ...
        if reference is not None:
            reference.sc_categories.all().delete()
    ...
    def get_categories_for_file(self, filename: str) -> List[str]:
        ...
        if reference is None:
            return []
        return [category.category for category in reference.sc_categories.all()]
    ...
    def add_categories(self, images: List[str], categories: List[str]) -> CategoriesFile:
        ...
        for image in images:
            ...
            for category in categories:
                if not reference.sc_categories.with_category(category).exists():
                    instance = Category(
                        file=reference,
                        category=category
                    )
    ...
    def remove_categories(self, images: List[str], categories: List[str]) -> CategoriesFile:
        ...
        for image in images:
            ...
            for category in categories:
                qs = reference.sc_categories.with_category(category)
                if qs.exists():
                    qs.delete()
